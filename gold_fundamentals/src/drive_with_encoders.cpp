...
void sensorCallback(const create_fundamentals::SensorPacket::ConstPtr &msg) {
    ...
    robot.encoderLeft = msg->encoderLeft;
    robot.encoderRight = msg->encoderRight;
}
...
void drive(double distance) {
    ...
    PID pid = PID(1.0 / RATE, Robot::MAX_SPEED, -Robot::MAX_SPEED, 1.0, 0.0, 0.5);

    double setpoint = robot.encoderLeft + distance * Robot::ENCODER_STEPS_PER_REVOLUTION / (M_PI * 2.0 * Robot::WHEEL_RADIUS);
    double pv = robot.encoderLeft;
    double prev_encoder = robot.encoderLeft;
    double out = 1.0;
